...
class sModel(Model):
    def fit_generator(self, generator, steps_per_epoch, epochs, validation_data, validation_steps, opt):
        ...
        for epoch in range(epochs):
            ...
            retv = self.evaluate_generator(validation_data, validation_steps)
            val_loss = exp(retv[0])
            val_losses.append(val_loss)
            print 'Epoch {}/{}. Validation loss: {}. Accuracy: {}'.format(epoch + 1, epochs, val_loss, retv[1])
...
#                 print 'lr = {}'.format(lr)
    ...
    @property
    def states_value(self):
        return [K.get_value(a[0]) for a in self.state_updates]

    def set_states_value(self, states):
        for a, state in zip(self.state_updates, states):
            K.set_value(a[0], state)

    def reindex_states(self, index):
        for a in self.state_updates:
            K.set_value(a[0], K.get_value(a[0])[index])
